...
class CompliantBackend:
    """Calls a tool while it is allowed to, and answers when it is not.
    ...
    Compliance is `tool_choice` now, not the presence of `tools` (ADR-0057). The tools stay
    in the prompt on the final turn -- withdrawing them cost the whole prefix cache -- so a
    double keyed on their absence would model a backend that no longer exists.
    """
...
class IgnoresTheWithdrawal:
    """Returns a tool call on every turn, including one forbidden to make any.

    The name is kept: what it ignores is the final-turn prohibition, whichever way that is
...
def test_a_compliant_backend_that_exhausts_the_budget_reports_the_limit(registered):
    ...
    assert backend.requests[-1].tool_choice == "none", "the last turn must forbid calls"
    assert backend.requests[-1].tools != (), (
        "and must keep offering them -- withdrawing moves the first difference to the front "
        "of the prompt and costs the whole prefix cache (ADR-0057)"
    )
...
def test_an_answer_before_the_last_turn_does_not_report_the_limit(registered):
    ...
    # One turn, and it is the final one, so calls are forbidden from the start.
    assert result.hit_turn_limit is True
