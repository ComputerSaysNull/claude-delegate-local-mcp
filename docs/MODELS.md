...
| `concurrency` | no | *This endpoint's* limit, checked alongside the global in-flight cap. Defaults to the global in-flight cap: an entry that says nothing must not bound itself below what the server will admit |
