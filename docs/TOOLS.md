...
| `path` | string | yes | Absolute path to a directory or file to search, and the only argument that narrows the walk. Required. Pass the exact string '_unscoped_' to walk every workspace root -- the slowest call this server offers, by roughly two orders of magnitude. |
